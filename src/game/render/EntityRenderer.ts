...
export class EntityRenderer {
    static drawEnemies(rc: any, ctx: CanvasRenderingContext2D, enemies: Enemy[]) {
        for (let enemy of enemies) {
            ...
            if (isAttacking && enemy.attackTimer < 20 && enemy.attackTimer > 5) {
                ...
                if (isRanged) {
                    // 远程怪攻击发光特效
                    rc.circle(20, 0, 25, { fill: '#c084fc', fillStyle: 'solid', stroke: 'none' });
                } else {
            ...
            if (isRanged) {
                rc.polygon([[-20, -60], [20, -60], [20, -20], [-20, -20]], { fill: headColor, fillStyle: 'solid', roughness: roughLevel, stroke: strokeColor, strokeWidth: 2 });
            } else {
                rc.circle(0, -35, 30, { fill: headColor, fillStyle: 'solid', roughness: roughLevel, stroke: strokeColor, strokeWidth: 2 });
            }
    ...
    static drawNPCs(rc: any, ctx: CanvasRenderingContext2D, hero: any, npcLiJing: any, npcTaiyi: any, dialogueActive: boolean) {
        ...
        if (distLi < 100 && !dialogueActive && hero.state !== 'UPGRADING') {
            ...
            ctx.fillText('[按 F 交互]', nx, ny - 70); // 坐标同步微调
        }
        ...
        if (distTaiyi < 100 && !dialogueActive && hero.state !== 'UPGRADING') {
            ctx.fillStyle = '#1f2937';
            ctx.font = 'bold 16px "Comic Sans MS", cursive, sans-serif';
            ctx.fillText('[按 F 修炼]', tx, ty - 90); // 坐标同步微调
        }
        
        ctx.textAlign = 'left'; // 恢复文本对齐状态
    }
}